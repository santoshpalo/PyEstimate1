...
if __name__ == "__main__":
    ...
    cutoff=cl.Quantity([['cut-off walls',1,144.5,.2]])
    print(cutoff.tba()['y0'],'{:.2f}sqm'.format(cutoff.tba()['y1']))
    print(it.items['efhs'])
    foundation = cl.Quantity([['cut-off walls',2,144.5,0.2,0.2]])
    foundation.rate = 103.2
    foundation.volume()
    print(it.items['sand_fill'])
    sand_fill = cl.Quantity([['road subgrade',1,298.31-144.5*.2,.05]])
    sand_fill.rate = 313.52
    sand_fill.aVolume()
    print(it.items['CC(1:3:6)'])
    metal_concrete = cl.Quantity([['cut-off walls',2,28.9,0.25],
                                  ['sub-base concrete',1,298.31,0.09]
                                  ])
    metal_concrete.rate = 3700.47
    metal_concrete.aVolume()
    print(it.items['CC(1:2:4)'])
    chips_concrete = cl.Quantity([['crust of the road',1,298.31,0.09]])
    chips_concrete.rate =4814.96
    chips_concrete.aVolume()
    print(it.items['rscs_plinth'])
    cut_off = cl.Quantity([['outside cut-off',2,144.5,0.05+.2],
                           ['inside cut-off',2,144.5,0.05]])
    cut_off.rate = 82.08
    cut_off.vArea()
    print('\nCost towards hire and running of plate vibrator')
    print('\n\t\t\t12.70 hour @ \u20B9 106.00/ hour = \u20B9 1346.00')
